...
$( document ).ready(function() {
	$.getJSON(requestLink(), function (data) {
		...
		var url = photo.url_o;
		var backup = "http://farm4.staticflickr.com/3694/10909068136_e5c5425378_o.jpg"; //If JSON request fails, then this photo url will be used.
...
function requestLink() {
	return "http://api.flickr.com/services/rest/?method=flickr.photos.search&content_type=1&text=" + "sunset" + "&format=json&tagmode=all&nojsoncallback=1&per_page=500&extras=url_o&api_key=" + FlickrAPIKey; //Flickr JSON request link
}

function findNumber() {
	return "http://www.random.org/sequences/?min=0&max=47&col=1&format=plain&rnd=new"; //Number generator API link
}
